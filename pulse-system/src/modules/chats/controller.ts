...
export class ChatController {
  createChat = async (req: any, res: any, next: any) => {
    try {
      ...
      if (req.user.role !== 'director') throw new AppError('Only directors can create chats', 403);
  ...
  getChatById = async (req: any, res: any, next: any) => {
    try {
      const chat = await prisma.chat.findUnique({
        ...
        include: { members: { include: { user: true } }, messages: { take: 50, orderBy: { createdAt: 'asc' } } }
      });
